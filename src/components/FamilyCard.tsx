...
export function FamilyCard({ family }: { family: Family }) {
  ...
  return (
    <article className="group relative rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm hover:shadow-lg hover:border-slate-300 transition-all duration-200 flex flex-col">
      {/* Header */}
      ...
      <div className="mt-auto pt-3 border-t border-slate-100">
        ...
        ) : family.donation_platform === "Venmo" || family.donation_platform === "CashApp" ? (
          <div className={`rounded-xl px-4 py-2.5 text-center text-sm font-medium ${style.bg} ${style.text}`}>
            {family.donation_platform}: <span className="font-bold">{family.donation_link}</span>
          </div>
        ) : (
          <div className="rounded-xl bg-slate-50 px-4 py-2.5 text-center text-sm text-slate-500">
            Search <span className="font-semibold text-slate-700">&ldquo;{family.name}&rdquo;</span> on {family.donation_platform || "GoFundMe"}
          </div>
        )}
      </div>
    </article>
